...
const getPLANS = (L) => [
  {
    ...
    features: [L('每月 3 次基础互动体验', '3 basic interactions/month'), L('标准语音与情绪反馈', 'Standard voice & emotion feedback'), L('基础硬件联动测试', 'Basic hardware sync test')],
  
  },
  {
    id: 'plus',
    name: L('沉浸互动 (Plus)', 'Immersive (Plus)'),
    price: '$9.99',
    period: L('月', 'mo'),
    tag: L('热门选择', 'Popular'),
    color: '#E35D8E',
    ...
    features: [
      L('每月 10 次高阶互动体验', '10 advanced interactions/month'),
      L('更细腻的语音与情绪表达', 'Richer voice & emotion expression'),
      L('进阶硬件同步反馈', 'Advanced hardware sync feedback'),
      L('个性化偏好定制', 'Personalized preference settings'),
    ],
  },
  {
    id: 'premium',
    name: L('专属定制 (Premium)', 'Custom (Premium)'),
    price: '$19.99',
    period: L('月', 'mo'),
    tag: null,
    color: '#7C88FF',
    ...
    features: [
      L('高级专属角色与情景库', 'Exclusive characters & scenarios'),
      L('无上限互动时长与次数', 'Unlimited interactions'),
      L('24h 优先响应与专属通道', '24h priority support & VIP channel'),
      L('全维度感官联动控制', 'Full sensory sync control'),
    ],
  },
]

const getWHY_ITEMS = (L) => [
  { Icon: Flame, color: '#E35D8E', title: L('更懂你的专属陪伴', 'A companion who truly knows you'), desc: L('根据你的偏好持续优化互动节奏与语气，体验更自然、更贴近。', 'Continuously optimizes rhythm and tone based on your preferences.') },
  { Icon: Zap, color: '#7C88FF', title: L('稳定顺滑的设备联动', 'Smooth & stable device sync'), desc: L('语音、文本与硬件反馈低延迟协同，让每次互动都更完整。', 'Low-latency sync of voice, text, and hardware for a seamless experience.') },
]
...
export default function SubscriptionPage() {
  ...
  const WHY_ITEMS = getWHY_ITEMS(L)

  const handleUnlock = () => {
    showToast(L('已开通专属计划，立即生效', 'Plan activated successfully'))
    setTimeout(() => {
      navigate(-1)
    }, 1500)
  }
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#0E1117] text-[#F5F7FF]">
      <HeaderBar title={L("专属会员计划", "Membership Plans")} onBack={() => navigate(-1)} />
      ...
      <div className="sticky bottom-0 left-0 w-full p-4 bg-gradient-to-t from-[#0E1117] via-[#0E1117] to-transparent">
        <SlideToUnlock onUnlock={handleUnlock} text={L("👉 向右滑动，立即开通...", "👉 Slide right to activate...")} />
        <p className="text-center text-[10px] text-[#9AA4BA]/70 mt-3">
          {L('开通后可在账户中随时管理套餐与续费设置。', 'Manage your plan and renewal in account settings anytime.')}
        </p>
      </div>
    </div>
  )
